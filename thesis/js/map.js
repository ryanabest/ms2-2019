Promise.all([
  ...
  function handleStepEnter(response) {
    ...
    if (dataStep == 0) {
      ...
      resizeMapAllZones(0,0.65,1,0.675,0.5);
    }
  	else if (dataStep == 1) {
      ...
      resizeMapAllZones(1500,0.65,1,0.675,0.5);
  ...
  function handleResize() {
  	...
  	resizeMapAllZones(0,0.65,1,0.675,0.5); // fit map to bounding box of all holc zones
  ...
  function resizeSparkLines() {
    // spark lines
    let  sparkMargin = {top:10,right:15,bottom:20,left:50}
        ...
        sparkLines.each(function(f,i) { // for each of the two sparklines
          ...
          f.forEach((year) => { // for each year of data appended to that spark line,
            if (d==year.year) { // for the year I'm hovering over...
              let pctWhite = d3.format(",.0%")(year.pctWhite), // pctWhite formatted as a % with no decimals
              ...
              function tooltipText() { // for each metric, changes the text of html element with "tooltip" class to that year's metric
                ...
                pctWhiteTooltips.each(function (td,ti) {
                  if (ti===i) { d3.select(this).text(pctWhite).style('color',stroke); }
              ...
              function hoverCircles() {
                ...
                pctWhiteCircles.each(function(cd,ci) {
                  if (ci===i) { d3.select(this).attr('cy',sparkYpctWhite(year.pctWhite)).attr('cx',sparkX(year.year)).style('display','inline').style('fill',stroke).style('stroke',stroke);}
                });
  ...
  function resizeMapAllZones(time,screenSpaceHoriz,screenSpaceVert,centerpointHoriz,centerpointVert) {
    ...
    figureSvg.selectAll('path').transition().ease(d3.easeCircle).duration(time).attr('d',geoGenerator); // update all drawn paths to the new projection – geoGenerator is defined in index.html and takes projection as an argument
    figureSvg.selectAll('circle').transition().ease(d3.easeCircle).duration(time).attr('cx',d=>projection(d.latLng)[0]).attr('cy',d=>projection(d.latLng)[1]) //.attr('r',.005/w)  house map markers
  }
  ...
  function showExplore() {
    ...
    opacityHolcGrade('C','light');
    opacityHolcGrade('D','light');
    resizeMapAllZones(0,1,1,0.5,0.5); // recenter map on first load
  ...
  function undoPanAndZoomMap() {
    ...
    resizeMapAllZones(0,0.65,1,0.675,0.5); // put map back to default view all zones view
  }
