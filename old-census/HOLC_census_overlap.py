...
def match_HOLC(holc_zone,year,i):
    ...
    text_row = str(year)+','+str(holc_zone['properties']['borough'])+','+str(holc_zone['properties']['holc_id'])+','+str(len(census_matches))+','+str(time.time()-start_time)+','+str(datetime.datetime.now())
    text_row += '\n'
    f = open("HOLC/Logs/full_log.txt","a+")
    f.write(text_row)
...
g = open("HOLC/Logs/overall_log.txt","a+")
overall_text_row = 'total time: ' + str(time.time() - overall_start_time) + ' | datetime started: ' + str(datetime_started) + ' | datetime finished: ' + str(datetime.datetime.now())
overall_text_row += '\n'
g.write(overall_text_row)
